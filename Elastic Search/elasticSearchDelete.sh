...
#_delete_by_query to delete the documents that are fetched
curl -u $username:$password -X POST -H "Content-Type: application/json" -d "$query" "$endpoint/$index_name/_delete_by_query" > response.json
